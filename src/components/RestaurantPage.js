    ...
    const bookMyTable = async (restaurant) => {
        if (!user) {
            ...
            if (!bookingTime) {
                ...
                let response = await axios.post(api.GET_PAYMENT, { amount: document.getElementById("amount").innerText });
                ...
                <div className="col-5">
                    <div className="row">
                        {restaurant.images.map((image, index) => {
                            return <div key={index} style={{ height: 228 + 'px' }} className={"sideImgHolder col-lg-6"}>
                                <img className="sideImg" onClick={() => viewImages(restaurant?.images)} src={api.GET_IMAGES + image} alt="/mezban-images/no-image.png" />
                            </div>
                        })}
                    </div>
                </div>
